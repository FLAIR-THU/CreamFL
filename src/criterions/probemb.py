...
class MCSoftContrastiveLoss(nn.Module):
    ...
    def __init__(self, config, reduction='sum'):
        ...
        shift = config.init_shift * torch.ones(1, device="cuda:0" if torch.cuda.is_available() else "cpu")
        negative_scale = config.init_negative_scale * torch.ones(1, device="cuda:0" if torch.cuda.is_available() else "cpu")
